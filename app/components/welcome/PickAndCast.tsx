...
export default function PickAndCast() {
  ...
  const onDragEnd = (_: PointerEvent, info: PanInfo) => {
    ...
    const swipeUp = offset.y < -100 || velocity.y < -700; // up is negative

    // prioritize vertical if it dominates
    if (absY > absX && swipeUp) {
      if (currentPost) {
        ...
        <div className="px-6 pb-4">
          <div className="flex items-center gap-6">
            <button
              type="button"
              onClick={() => setTab('post')}
              className={`rounded-full px-6 py-3 font-outfit text-[18px] transition-all ${
                tab === 'post' ? 'bg-[#4a4653] text-white shadow-sm' : 'bg-transparent text-[#bfbfc4] hover:text-black'
              }`}
            >
              Post
            </button>
            ...
            <button
              ...
              className={`rounded-full px-6 py-3 font-outfit text-[18px] transition-all ${
                tab === 'story' ? 'bg-[#4a4653] text-white shadow-sm' : 'bg-transparent text-[#bfbfc4] hover:text-black'
              }`}
            >
              Story
            </button>
          ...
          <AnimatePresence mode="wait" initial={false} presenceAffectsLayout={false}>
            {currentPost && (
              <motion.div
                ...
                initial="enter"
                animate="center"
                drag // enable both axes for swipe up + left/right
                dragElastic={0.18}
                dragMomentum={false}
                style={{ x, y, rotate }}
                onDragEnd={onDragEnd}
