...
const USERNAME = process.env.NEXT_PUBLIC_DEFAULT_IG_USERNAME || 'india.base.eth';
...
export default function PickAndCast() {
    ...
    // keep "Story" tab in UI but disable it (we're only using /api/posts here)
    ...
    useEffect(() => { x.set(0); }, [index, x]);

    // API item -> UI Post
    const mapToPost = (m: ApiItem, i: number): Post => ({
        id: m.id, // stable key from backend
    ...
    // Call /api/posts correctly
    const loadPage = async (cursor: string | null) => {
        const res = await fetch('/api/posts', {
            method: 'POST',
            headers: { 'content-type': 'application/json' }, // <— IMPORTANT
            body: JSON.stringify({
                username_or_url: USERNAME,                      // <— field name backend expects
                after: cursor,
            }),
            cache: 'no-store',
        });
        ...
        const newPosts = (data.items || [])
            .filter((m) => !!m.thumbnail)            // drop empties to avoid empty <img src="">
            .map(mapToPost);

        // de-dupe by id, then sort newest-first (takenAt desc, fallback to keep existing order)
        setQueue((prev) => {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                await loadPage(null);
                if (alive) setNotice(`Loaded posts + reels for @${USERNAME}.`);
            } catch (e: any) {
                if (alive) setError(e?.message ?? 'Unable to load posts');
            } finally {
    ...
    useEffect(() => {
        if (index >= queue.length - 3 && hasMore) {
            loadPage(after).catch(() => { });
        }
    }, [index, queue.length, hasMore, after]);
    ...
    const flyOutLeft = async () => {
        ...
        advance();            // index++ will mount the next card
        // x resets to 0 in your useEffect([index]) – good
      };
      ...
      const onDragEnd = (_: PointerEvent, info: PanInfo) => {
        ...
        if (farLeft || fastLeft) {
          flyOutLeft();
          return;            // <-- avoid any accidental fallthrough
        }
        snapBack();
      };
    ...
    const handleCast = async () => {
        // Only reels will have "code"
        const code = currentPost?.code;
        if (!code) { advance(); return; }
        try {
    ...
    const primaryActionLabel = 'Post'; // Story disabled for now
    ...
    return (
        <div className="min-h-screen bg-[#e8e9eb] p-4">
            ...
            <div className="bg-white rounded-[32px] max-w-sm mx-auto overflow-hidden">
                ...
                {/* Segmented control (Story disabled) */}
                <div className="px-6 pb-4">
                    <div className="flex items-center gap-6">
                        ...
                        <button
                            type="button"
                            disabled
                            title="Stories not wired yet"
                            className="rounded-full px-6 py-3 font-outfit text-[18px] bg-transparent text-[#d0d0d0] cursor-not-allowed"
                            aria-pressed="false"
                ...
                <div className="px-6 select-none" style={{ touchAction: 'pan-y' }}>
                    ...
                    <AnimatePresence mode="wait" initial={false} presenceAffectsLayout={false}>
                        {currentPost && (
                            ...
                            >
                                ...
                                {/* Thumbnail (guard against empty src) */}
                    ...
                    {!loading && (!currentPost || index >= total) && (
                        <div className="bg-[#F8F8F8] rounded-3xl p-6 mb-6 text-center">
                            ...
                            <div className="text-[#666] text-sm">Try another Instagram username.</div>
                        </div>
                    )}
                </div>
                ...
                <div className="flex items-center justify-between px-6 py-4 border-t border-[#F0F0F0]">
                    ...
                    <button onClick={() => (currentPost ? handleCast() : undefined)} className="flex flex-col items-center gap-2 p-2 hover:bg-gray-50 rounded-lg transition-colors">
                        ...
                        <span className="text-[#666] font-outfit text-xs">Post</span>
                    </button>
                </div>

                <div className="text-center py-3 text-[#666] font-outfit text-sm border-t border-[#F0F0F0]">
                    {useMemo(() => (queue.length ? `${Math.min(index + 1, queue.length)} of ${queue.length}` : '0 of 0'), [index, queue.length])}
                </div>

                {/* Bottom nav */}
                <div className="flex items-center justify-around py-4 border-t border-[#F0F0F0]">
                    <button className="w-10 h-10 bg-black rounded-full flex items-center justify-center hover:bg-gray-800 transition-colors" aria-label="Add">
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
